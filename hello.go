...
//
// 폴더명과 패키지 이름이 일치하는 일반적인 경우 패키지는,
// 공유 라이브러리로서, import 구문으로 외부에서 불러올 수 있고,
// 그 패키지의 이름으로 식별 가능한 형태다.
//
// 폴더명과 패키지의 이름 관계에는 한 가지 예외가 있다.
// 해당 폴더의 패키지의 이름을 폴더명으로 사용하지 않는 대신 main으로 할 수 있다.
//
// main 패키지에는 특수한 규칙이 있다.
// 1) main 패키지의 main()은 전체 프로그램의 엔트리 포인트로 취급된다.
// 2) main 패키지는 라이브러리가 아니기 때문에 외부 프로그램에서 import 구문으로 불러올 수 없다.
//
// 한 패키지 내에는 여러 가지 이름의 .go 파일들이 존재할 수 있다.
// 하지만 한 패키지 내에 같은 이름의 함수는 하나씩밖에 존재할 수 없다.
//
// .go 파일의 이름은 패키지나 폴더명과 관계 없이 자유롭게 작명 가능하다.
// .go 파일의 이름이 문제가 되는 경우는 go test로 실행하는 유닛 테스트를 작성하는 경우뿐이다.
package main
...
func main() {
	...
	// Go에서 &는 new와 같다. !!!!!!!!!!!!
	// (중요!): Go에서 &연산자는, 단순한 주소참조가 아니라, 여타 언어에서의 new 키워드의 의미로 쓰인다.
	// & == new
	...
	// (보통 파일처리시 쓰이는 스트림이랑 용도는 조금 다르지만 형상이 닮았다.)
	//
	// 중요성)
